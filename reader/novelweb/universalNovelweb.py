...
from novelweb.biquge import BiQuGe
...
from util.myUtil import remove_duplicate_part_of_string, FError
...
class Novelweb:
    ...
    webDomain_biquge = "www.bequgew.com"
    webDomain_mianhuatang = "www.mianhuatang520.com"
    ...
    def parse_homeUrl(self, url):
        # html, url = self.myRequests.get_html_and_linkUrl(url=url)
        # 通过链接的Url判断在哪个网站爬
        if self.webDomain_piaotian in url:
            print('飘天文学网')
            self.web_domain = self.webDomain_piaotian
            return PiaoTianWenXue().parse_homeUrl(url, web_domain=self.web_domain)
        elif self.webDomain_bayi in url:
            print("八一中文网")
            self.web_domain = self.webDomain_bayi
            return BaYiZhongWen().parse_homeUrl(url, web_domain=self.web_domain)
        elif self.webDomain_biquge in url:
            print("笔趣阁")
            self.web_domain = self.webDomain_biquge
            return BiQuGe().parse_homeUrl(url, web_domain=self.web_domain)
        elif self.webDomain_mianhuatang in url:
            print("棉花糖")
            self.web_domain = self.webDomain_mianhuatang
            return MianHuaTang().parse_homeUrl(url, web_domain=self.web_domain)
        # elif self.webDomain_dingdian in url:
        #     print("顶点小说")
        #     self.web_domain = self.webDomain_dingdian
        #     return DingDian().parse_homeUrl(url, html, self.web_domain)
        else:
            self.web_domain = ""
            html, url = self.myRequests.get_html_and_linkUrl(url=url)
            return self.usual_parse_homeUrl(url, html)
    ...
    def parse_chapterHtml(self, html):
        ...
        elif self.web_domain == self.webDomain_biquge:
            return BiQuGe().parse_chapterHtml(html)
        elif self.web_domain == self.webDomain_mianhuatang:
            return MianHuaTang().parse_chapterHtml(html)
        else:
